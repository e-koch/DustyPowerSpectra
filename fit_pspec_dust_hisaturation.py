...
dust_satpt = (hi_satpt / dgr) / np.cos(55 * u.deg).value
...
pspec = PowerSpectrum(fits.PrimaryHDU(proj_coldens, hdr), distance=840 * u.kpc)
pspec.run(verbose=False, fit_2D=False)


pspec_sat = PowerSpectrum(fits.PrimaryHDU(dust_sat, hdr), distance=840 * u.kpc)
...
plt.loglog(phys_scales, pspec_sat.ps1D, 'k', zorder=-10)
