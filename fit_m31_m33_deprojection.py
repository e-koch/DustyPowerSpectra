...
'''
Deproject the M33 and M31 SPIRE 250 maps and compare to the originals.
'''
...
fitinfo_dict["M33"] = {'filename': r"m33_spire500_mod_mjysr_cutout.fits",
                       'beam': Beam(41.0 * u.arcsec), 'apod_kern': None,
                       'low_cut': None, 'high_cut': None,
                       'distance': 840 * u.kpc}
fitinfo_dict["M31"] = {'filename': r"m31_spire500_mod_mjysr_cutout.fits",
...
for gal in (['M33', 'M31']):
    ...
    deproj_img = deproject(proj, proj.header,
    ...
    deproj_beam = Beam(major=proj.beam.major / np.cos(inc).value,
                       minor=proj.beam.major,
                       pa=90 * u.deg)

    pixscale = np.abs(proj_plane_pixel_scales(proj.wcs.celestial)[0]) * u.deg
    ...
    save_name = "{0}_{1}_{2}_mjysr_deproj.pspec.pkl".format(gal.lower(),
                                                            'spire500',
                                                            res_type)

    pspec_dep.save_results(osjoin(data_path, gal, save_name))
    ...
    plot_savename = osjoin(plot_folder, "{0}_spire500.deproj.pspec_wbeam_traceplot.png".format(gal))
    ...
    plot_savename = osjoin(plot_folder, "{0}_spire500_deprojcompare.1Dpspec_wbeam.png".format(gal))
    plt.savefig(plot_savename)
    plot_savename = osjoin(plot_folder, "{0}_spire500_deprojcompare.1Dpspec_wbeam.pdf".format(gal))
    plt.savefig(plot_savename)

    plt.close()

df = pd.DataFrame(fit_results, index=row_names)
df.to_csv(osjoin(data_path, "pspec_spire500_deproj_fit_results.csv"))
